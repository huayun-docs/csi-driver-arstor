...
# - running the stable arstor example against a Kubernetes release
# - running the canary arstor example against a Kubernetes release
# - building the component in the current repo and running the
#   stable arstor example with that one component replaced against
...
# The arstor deployment script is searched for in several places.
#
...
#   csi-driver-arstor itself provides the example.
#
...
#
# - The csi-driver-arstor/deploy directory has multiple sub-directories,
...
#   "none" disables the deployment of the arstor driver.
#
# When no deploy script is found (nothing in `deploy` directory,
# CSI_PROW_HOSTPATH_REPO=none), nothing gets deployed.
configvar CSI_PROW_HOSTPATH_VERSION "v1.2.0-rc2" "arstor driver"
configvar CSI_PROW_HOSTPATH_REPO https://github.com/kubernetes-csi/csi-driver-arstor "arstor repo"
configvar CSI_PROW_DEPLOYMENT "" "deployment"
configvar CSI_PROW_HOSTPATH_DRIVER_NAME "arstor.csi.k8s.io" "the arstor driver name"

# If CSI_PROW_HOSTPATH_CANARY is set (typically to "canary", but also
# "1.0-canary"), then all image versions are replaced with that
# version tag.
configvar CSI_PROW_HOSTPATH_CANARY "" "arstor image"
...
configvar CSI_PROW_SANITY_SERVICE "arstor-service" "Kubernetes TCP service name that exposes csi.sock"
configvar CSI_PROW_SANITY_POD "csi-arstorplugin-0" "Kubernetes pod with CSI driver"
configvar CSI_PROW_SANITY_CONTAINER "arstor" "Kubernetes container with CSI driver"
...
find_deployment () {
    ...
    if [ "${CSI_PROW_DEPLOYMENT}" ]; then
        file="$dir/${CSI_PROW_DEPLOYMENT}/deploy-arstor.sh"
        if ! [ -e "$file" ]; then
            return 1
        fi
        echo "$file"
    ...
    # Ignore: See if you can use ${variable//search/replace} instead.
    # shellcheck disable=SC2001
    file="$dir/kubernetes-$(echo "${CSI_PROW_KUBERNETES_VERSION}" | sed -e 's/\([0-9]*\)\.\([0-9]*\).*/\1.\2/')/deploy-arstor.sh"
    if ! [ -e "$file" ]; then
        file="$dir/kubernetes-latest/deploy-arstor.sh"
        if ! [ -e "$file" ]; then
            return 1
        fi
    fi
...
# This installs the arstor driver example. CSI_PROW_HOSTPATH_CANARY overrides all
# image versions with that canary version. The parameters of install_arstor can be
...
install_arstor () {
    ...
    if deploy_arstor="$(find_deployment "$(pwd)/deploy")"; then
        :
    elif [ "${CSI_PROW_HOSTPATH_REPO}" = "none" ]; then
        return 1
    else
        git_checkout "${CSI_PROW_HOSTPATH_REPO}" "${CSI_PROW_WORK}/arstor" "${CSI_PROW_HOSTPATH_VERSION}" --depth=1 || die "checking out arstor repo failed"
        if deploy_arstor="$(find_deployment "${CSI_PROW_WORK}/arstor/deploy")"; then
            :
        else
            die "deploy-arstor.sh not found in ${CSI_PROW_HOSTPATH_REPO} ${CSI_PROW_HOSTPATH_VERSION}. To disable E2E testing, set CSI_PROW_HOSTPATH_REPO=none"
        fi
    fi
    ...
    # shellcheck disable=SC2086
    if ! run env $images "${deploy_arstor}"; then
        ...
        die "deploying the arstor driver with ${deploy_arstor} failed"
    fi
}
...
docker exec csi-prow-control-plane docker image ls --format='{{.Repository}} {{.Tag}} {{.ID}}' | grep -e csi -e arstor | while read -r repo tag id; do
...
# Whether the arstor driver supports raw block devices depends on which version
...
arstor_supports_block () {
    local result
    result="$(docker exec csi-prow-control-plane docker image ls --format='{{.Repository}} {{.Tag}} {{.ID}}' | grep arstor | while read -r repo tag id; do
        if [ "$tag" == "v1.0.1" ]; then
...
# driver test configuration for the arstor driver.
#
# The content depends on both what the E2E suite expects and what the
# installed arstor driver supports. Generating it here seems prone
...
run_e2e () (
    ...
    # TODO (?): multi-node cluster (depends on https://github.com/kubernetes-csi/csi-driver-arstor/pull/14).
    # When running on a multi-node cluster, we need to figure out where the
    # arstor driver was deployed and set ClientNodeName accordingly.
...
main () {
    ...
    if tests_need_kind; then
        ...
        if ${CSI_PROW_BUILD_JOB}; then
            ...
            # deploy-arstor.sh env variables for it. We also need to
